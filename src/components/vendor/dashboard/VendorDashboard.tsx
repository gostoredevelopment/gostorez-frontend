...
// Helper function for formatting price - defined outside component
...
const VendorDashboard: React.FC = () => {
  ...
  // Fetch user profile like other pages
  ...
  // Fetch vendor stats AND balance from Supabase ONLY
  const fetchSupabaseStats = async (vendorId: string) => {
    try {
      ...
      console.error('Error in fetchSupabaseStats:', error);
      setSupabaseStats({
        followers_count: 0,
        likes_count: 0,
        sales_count: 0,
  ...
  // Fetch products from Supabase for the selected shop
  ...
  useEffect(() => {
    if (activeTab === 'products' && vendorProfile) {
  ...
  // Calculate total stats from products
  const getTotalStats = () => {
    const activeProducts = products.filter(p => p.is_active).length;
    const totalRevenue = products.reduce((sum, product) => 
      sum + (product.price * product.sales_count), 0
    );
    const totalViews = products.reduce((sum, product) => sum + product.views_count, 0);
    const totalLikes = products.reduce((sum, product) => sum + product.likes_count, 0);
  ...
  return (
    <div className="vendash-container">
      {/* Header with user profile display and buttons in correct order */}
      ...
      <main className="vendash-main-content">
        ...
        <section className="vendash-shop-background-section">
          ...
          <div className="vendash-shop-overlay-content">
            {/* Shop Identity Card with white background */}
        ...
        {/* Balance Overview Section with white background */}
        ...
        <div className="vendash-tab-content-area">
          {activeTab === 'overview' && (
            <OverviewTab 
              stats={stats} 
              products={products} 
              onAddProduct={handleAddProductRedirect}
      ...
      {/* Bottom Navigation with icons only, max height 35px */}
      <nav className="vendash-bottom-navigation-bar">
        <button 
...
const OverviewTab: React.FC<{
  ...
  onViewProducts: () => void;
  onViewOrders: () => void;
}> = ({ stats, products, onAddProduct, onViewProducts, onViewOrders }) => {
    ...
    <div className="vendash-overview-tab">
      <div className="vendash-stats-grid-overview">
        ...
        <div className="vendash-stat-card-item">
          ...
          <div className="vendash-stat-details-content">
            <h3>Total Sales</h3>
            <p className="vendash-stat-number-value">{stats.totalSales}</p>
          </div>
        </div>
        <div className="vendash-stat-card-item">
          <div className="vendash-stat-icon-wrapper">
        ...
        <div className="vendash-stat-card-item">
          ...
          <div className="vendash-stat-details-content">
            ...
            <p className="vendash-stat-number-value">{stats.activeProducts}</p>
          </div>
        </div>
        <div className="vendash-stat-card-item">
          <div className="vendash-stat-icon-wrapper">
          ...
          <div className="vendash-stat-details-content">
            ...
            <p className="vendash-stat-number-value">
              {stats.totalViews > 0 ? ((stats.totalSales / stats.totalViews) * 100).toFixed(1) : '0'}%
            </p>
          </div>
        </div>
      </div>
            ...
            {products.slice(0, 4).map(product => (
              <div key={product.id} className="vendash-product-card-item">
                ...
                <div className="vendash-product-info-content">
                  ...
                  <div className="vendash-product-revenue-text">
                    Revenue: {formatPrice(product.price * product.sales_count)}
                  </div>
                </div>
              </div>
            ))}
...
// UPDATED: ProductsTab with rectangular format like FavoritesPage
const ProductsTab: React.FC<{
  ...
  return (
    <div className="vendash-products-tab">
      ...
      {/* Product Controls - Like FavoritesPage */}
      ...
      {/* Products Display - Rectangular format like FavoritesPage */}
      {filteredProducts.length === 0 ? (
        ...
        <div className={`vendash-products-display ${viewMode === 'grid' ? 'vendash-products-grid-view' : 'vendash-products-list-view'}`}>
          {filteredProducts.map((product) => (
            <div 
              key={product.id} 
              className="vendash-product-item-rectangular"
            ...
            >
              ...
              <div className="vendash-product-image-container">
                ...
                {/* Status Badge */}
                <div 
                  className="vendash-product-status-badge"
                  style={{ backgroundColor: getStatusColor(product.is_active) }}
                >
              ...
              <div className="vendash-product-info-section">
                ...
                {/* Product Stats - Like FavoritesPage */}
                <div className="vendash-product-stats-row">
...
const OrdersTab: React.FC = () => (
  <div className="vendash-orders-tab">
    <div className="vendash-section-header-row">
      <h3 className="vendash-section-title-heading">Order Management</h3>
    </div>
    <div className="vendash-empty-state-container">
      <div className="vendash-empty-icon-container">
        <PackageCheck size={24} />
      </div>
      <h4>Orders Will Appear Here</h4>
      <p>When customers purchase your products, orders will appear here</p>
      <button 
        className="vendash-cta-primary-button"
        onClick={() => window.location.href = '/marketplace'}
      >
        <ShoppingCart size={12} /> Promote Products
      </button>
    </div>
  </div>
);
...
const TransactionsTab: React.FC = () => (
  <div className="vendash-transactions-tab">
    ...
    <div className="vendash-empty-state-container">
      ...
      <button 
        className="vendash-cta-primary-button"
        onClick={() => window.location.href = '/vendor/balance'}
      >
        <DollarSign size={12} /> View Balance Details
      </button>
    </div>
  </div>
);
